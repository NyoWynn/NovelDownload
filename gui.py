...
class NovelDownloaderApp(ctk.CTk):
    def __init__(self):
        ...
        self._format_mode = tk.StringVar(value="pdf")
        ...
        self._log(
            "Sitios soportados: matrone-scan.xyz y cualquier sitio con tema WordPress Madara."
        )
        self._log("Pega la URL de la novela y presiona Obtener info.")
    ...
    def _build_ui(self):
        ...
        self._url_entry = ctk.CTkEntry(
            url_card,
            placeholder_text=(
                "https://matrone-scan.xyz/manga/efdzdcr/  "
                "o  https://web.archive.org/web/.../lunarletters.com/manga/..."
            ),
        ...
        self._fetch_btn = ctk.CTkButton(
            btn_row,
            text="\U0001f50d  OBTENER INFO",
            height=40,
            width=175,
            font=ctk.CTkFont("Arial Black", 13),
            fg_color=ACCENT,
        ...
        format_row.grid(row=5, column=0, columnspan=3, padx=14, pady=(0, 4), sticky="w")

        ctk.CTkRadioButton(
            format_row,
            text="PDF",
            variable=self._format_mode,
            value="pdf",
            text_color=TEXT_PRIMARY,
            border_color=BORDER,
            fg_color=ACCENT,
        ...
        ctk.CTkLabel(
            ...
            text_color=TEXT_MUTED,
            font=ctk.CTkFont(size=11, weight="bold"),
        ).grid(row=6, column=0, columnspan=3, padx=14, pady=(6, 2), sticky="w")

        language_row = ctk.CTkFrame(opts, fg_color="transparent")
        language_row.grid(row=7, column=0, columnspan=3, padx=14, pady=(0, 8), sticky="w")
        ...
        out_frame.grid(row=8, column=0, columnspan=3, padx=14, pady=(4, 6), sticky="ew")
        ...
        self._download_btn = ctk.CTkButton(
            ...
            height=50,
            width=220,
            font=ctk.CTkFont("Arial Black", 15),
            text_color="white",
        ...
        self._download_btn.pack(side="left", padx=(0, 14))
    ...
    def _on_fetch(self):
        ...
        corrected = _normalize_novel_url(url)
        if corrected != url:
            self._url_entry.delete(0, "end")
            self._url_entry.insert(0, corrected)
            self._log(
                f"⚠ URL de capítulo detectada. Ajustada a la página de la novela:\n   {corrected}",
                color=WARNING,
            )
            url = corrected
    ...
    def _fetch_thread(self, url: str):
        try:
            ...
            self.after(0, lambda e=str(exc): self._on_fetch_error(e))
    ...
    def _on_download(self):
        ...
        threading.Thread(
            ...
            args=(chapters, self._language_mode.get(), self._format_mode.get()),
            daemon=True,
        ).start()

    def _download_thread(self, chapters: list[ChapterInfo], output_language: str, output_format: str = "pdf"):
        ...
        try:
            ...
            ext = "epub" if output_format == "epub" else "pdf"
            out_path = os.path.join(
                self._output_dir,
                f"{safe_title}{suffix}_{timestamp}.{ext}",
            )

            self.after(
                0,
                lambda f=ext: self._log(
                    f"Generando {f.upper()}... esto puede tardar un momento.",
                    color=ACCENT[1],
                ),
            )
    ...
    def _show_help(self):
        ...
        win.title("Ayuda — Novel Downloader")
        win.geometry("580x560")
        win.resizable(False, False)
        win.configure(fg_color=SURFACE_BG)
        win.grab_set()

        ctk.CTkLabel(
            win,
            text="Sitios soportados y consejos de uso",
            font=ctk.CTkFont("Arial Black", 15),
            text_color=TEXT_PRIMARY,
        ).pack(pady=(22, 4), padx=24, anchor="w")

        ctk.CTkFrame(win, height=2, fg_color=BORDER).pack(fill="x", padx=24, pady=(0, 14))
        ...
        HELP_TEXT = """SITIOS SOPORTADOS
─────────────────────────────────────
• matrone-scan.xyz / matronescan.xyz
• Cualquier sitio con tema WordPress Madara
• URLs de Wayback Machine (web.archive.org)


FORMATO CORRECTO DE URL
─────────────────────────────────────
Debes pegar la URL de la PÁGINA PRINCIPAL de la novela,
no la URL de un capítulo específico.

  ✓ CORRECTO:
     https://matrone-scan.xyz/manga/mi-novela/

  ✗ INCORRECTO (URL de capítulo):
     https://matrone-scan.xyz/manga/mi-novela/libro-4/episodio-195/

  → La app corrige esto automáticamente al presionar
    "Obtener info", pero es bueno saberlo.


WAYBACK MACHINE
─────────────────────────────────────
Para sitios archivados, la URL tiene este formato:
  https://web.archive.org/web/20250101000000/https://sitio.com/manga/titulo/

La parte importante es que termine en la página índice
de la novela, no en un capítulo.


RANGO DE CAPÍTULOS
─────────────────────────────────────
Puedes descargar solo un rango en vez de todos:
  • Selecciona "Rango" en Opciones de descarga
  • Escribe el número del capítulo inicial y final
  • Ej: del cap. 1 al 50 → descarga solo esos 50


TRADUCCIÓN
─────────────────────────────────────
La opción "Español" traduce automáticamente el contenido
usando Google Translate. Requiere conexión a internet
y puede tardar más según la cantidad de capítulos.


FORMATOS DE SALIDA
─────────────────────────────────────
• PDF  → ideal para leer en PC o imprimir
• ePub → ideal para lectores (Kindle, Kobo, Apple Books)
"""
...
def _normalize_novel_url(url: str) -> str:
    ...
    wb_match = re.match(r"(https?://web\.archive\.org/web/\d+(?:if_)?/)(https?://.+)", url)
    if wb_match:
        prefix = wb_match.group(1)
        original = wb_match.group(2)
        normalized_original = _normalize_novel_url(original)
        return prefix + normalized_original if normalized_original != original else url

    parsed = urlparse(url)
    parts = [p for p in parsed.path.split("/") if p]

    for keyword in ("manga", "serie", "series", "novel", "novela"):
        if keyword in parts:
