...
# Algunos sitios tienen certificados SSL mal configurados (ej. matronescan.xyz).
# Desactivamos la advertencia para no saturar el log.
...
class NovelScraper:
    def __init__(
        ...
        self.session.verify = False  # Permite sitios con SSL mal configurado
    ...
    def _extract_chapter_list(
        ...
        chapters = self._parse_chapter_items(soup, base_url, timestamp)

        # For direct (non-Wayback) Madara sites like eternalune.com, the initial
        # HTML only loads ~15 chapters.  Fetch the full list via AJAX.
        if len(chapters) < 30 and not timestamp:
            ajax_chapters = self._fetch_chapters_ajax(base_url)
            if ajax_chapters and len(ajax_chapters) > len(chapters):
                self._log(f"AJAX: encontrados {len(ajax_chapters)} capítulos (HTML inicial: {len(chapters)})")
    ...
    def _parse_chapter_items(
        ...
        """Parse chapter <li> items from a BeautifulSoup fragment."""
    ...
    def _fetch_chapters_ajax(self, novel_url: str) -> list[ChapterInfo] | None:
        """Fetch the complete chapter list via the Madara AJAX endpoint."""
        ajax_url = novel_url.rstrip("/") + "/ajax/chapters/"
        try:
            self._log("Cargando lista completa de capítulos vía AJAX...")
            resp = self.session.post(ajax_url, timeout=30)
            if resp.ok and resp.text.strip():
                time.sleep(self.delay)
                ajax_soup = BeautifulSoup(resp.text, "lxml")
                return self._parse_chapter_items(ajax_soup, novel_url, None)
        except requests.RequestException as exc:
            self._log(f"AJAX capítulos no disponible: {exc}")
        return None
